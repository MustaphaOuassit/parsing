...
/*   Updated: 2021/12/14 06:43:33 by mouassit         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*fill_convert_two(char *dollar, t_envp *env, int *len, char *token_word)
{
	int	j;
	...
	if (dollar)
	{
		dollar = ft_strdup(get_env_couts(dollar, env));
		free_in_parcer(&env->allocation, dollar, NULL);
		while (dollar[j])
		{
...
char	*dollar_value(t_init *vr, int *len, int *i, t_envp *env_list)
{
	...
	vr->token_word = fill_convert_two(var.dollar, env_list, len,
			vr->token_word);
