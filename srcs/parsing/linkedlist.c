...
/*   Updated: 2021/12/16 23:43:43 by mouassit         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_tokens(t_list *head, int error, t_envp *env_list, t_data **dt)
{
	...
	initial_ex(&var.token, &var.token_word, &var.len_dollar, &var.dollar);
	while (head != NULL)
	{
		...
		while (var.j <= (int)ft_strlen(head->value))
		{
			...
			if (var.len)
				var.token_word = putex(&var.len, head->value, &var.j, env_list);
